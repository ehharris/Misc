...
int get_running_count()
{
    ...
    int count = 0;
    int count2 = 0;
    int numba = get_iteration_count(rand());
    //LoopdeLoop
    for(int i = 0; i < numba; i++){
        int arrsize = get_arr_size(rand());
        //Initialize array
            intray = (int *) malloc(arrsize * sizeof(int));
            for (int j = 0; j < arrsize; j++){
                intray[j] = rand();
            }
//         ascend(intray, arrsize);
//         printf("Bottom Before: %d\n", intray[0]);
            int median = return_median(intray, arrsize);
//         printf("bottom aft: %d\n", intray[0]);
